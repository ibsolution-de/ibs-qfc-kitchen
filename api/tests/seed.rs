...
async fn temp_pool() -> (SqlitePool, PathBuf) {
    let path = std::env::temp_dir().join(format!("qfc-seed-test-{}.db", uuid::Uuid::new_v4()));
    let pool = qfc_api::db::connect(path.to_str().expect("temp path is utf8"))
        .await
...
fn real_seed_json() -> String {
    std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/seed/seed.json")).expect("read seed/seed.json")
}

fn array_len(value: &serde_json::Value, key: &str) -> usize {
    value.get(key).and_then(serde_json::Value::as_array).map_or(0, Vec::len)
}
...
fn distinct_assignment_cells(version: &serde_json::Value) -> usize {
    ...
    for assignment in version.get("assignments").and_then(serde_json::Value::as_array).into_iter().flatten() {
        cells.insert((
            assignment["employeeId"].as_str().unwrap_or_default().to_string(),
            assignment["projectId"].as_str().unwrap_or_default().to_string(),
...
async fn seed_if_empty_populates_row_counts_matching_the_json_and_writes_no_change_log() {
    ...
    let expected: serde_json::Value = serde_json::from_str(&raw).expect("seed.json must be valid JSON");
    let expected_versions = expected["versions"].as_array().cloned().unwrap_or_default();
    let expected_assignments: usize = expected_versions.iter().map(distinct_assignment_cells).sum();
    let expected_absences: usize = expected_versions.iter().map(|v| array_len(v, "absences")).sum();
    let expected_quarter_data: usize = expected_versions.iter().map(|v| array_len(v, "forecastData")).sum();

    let seeded = seed_if_empty(&pool).await.expect("seed ok");
    assert!(seeded, "first call against an empty db must perform the seed");

    assert_eq!(count(&pool, "employee").await as usize, array_len(&expected, "employees"));
    assert_eq!(count(&pool, "project").await as usize, array_len(&expected, "projects"));
    assert_eq!(count(&pool, "customer").await as usize, array_len(&expected, "customers"));
    assert_eq!(count(&pool, "strategic_goal").await as usize, array_len(&expected, "goals"));
    assert_eq!(count(&pool, "north_star").await as usize, array_len(&expected, "northStars"));
    assert_eq!(count(&pool, "one_on_one").await as usize, array_len(&expected, "oneOnOnes"));
    assert_eq!(count(&pool, "public_holiday").await as usize, array_len(&expected, "holidays"));
    assert_eq!(count(&pool, "plan_version").await as usize, expected_versions.len());
    assert_eq!(count(&pool, "assignment").await as usize, expected_assignments);
    assert_eq!(count(&pool, "absence").await as usize, expected_absences);
    assert_eq!(count(&pool, "quarter_data").await as usize, expected_quarter_data);
    assert!(expected_assignments > 0, "sanity check: the fixture must actually carry assignments");

    assert_eq!(count(&pool, "change_log").await, 0, "seeding must never write change_log rows");

    let seeded_marker: Option<String> = sqlx::query_scalar("SELECT value FROM meta WHERE key = 'seeded'")
        .fetch_optional(&pool)
        .await
        .expect("query meta");
...
async fn second_call_is_a_no_op_and_does_not_duplicate_rows() {
    ...
    let seeded_again = seed_if_empty(&pool).await.expect("second call must not error");
    assert!(!seeded_again, "a database already marked seeded must not be re-seeded");
    assert_eq!(count(&pool, "employee").await, employees_after_first, "second call must not duplicate employee rows");
    assert_eq!(count(&pool, "assignment").await, assignments_after_first, "second call must not duplicate assignment rows");
...
async fn corrupted_seed_json_fails_and_writes_nothing() {
    ...
    let err = seed_from_json(&pool, "{ this is not valid json").await.expect_err("malformed JSON must error");
    ...
    assert_eq!(count(&pool, "meta").await, 0, "a rolled-back attempt must not leave a `seeded` marker behind");

    // Since the failed attempt never committed a `seeded` marker, a
    // subsequent good call must still be able to seed normally.
    let seeded = seed_from_json(&pool, &real_seed_json()).await.expect("seed ok after a prior failed attempt");
    assert!(seeded);
