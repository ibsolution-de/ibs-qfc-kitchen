...
use qfc_api::proto::admin::{
    AdminService, DeleteUserRequest, ListUsersRequest, UpsertUserRequest,
};
...
async fn temp_pool() -> (SqlitePool, PathBuf) {
    let path = std::env::temp_dir().join(format!("qfc-admin-test-{}.db", uuid::Uuid::new_v4()));
...
async fn list_users(svc: &AdminServiceImpl, ctx: RequestContext) -> Result<Vec<User>, ConnectError> {
...
async fn upsert_user(svc: &AdminServiceImpl, ctx: RequestContext, request: UpsertUserRequest) -> Result<User, ConnectError> {
...
async fn delete_user(svc: &AdminServiceImpl, ctx: RequestContext, email: &str) -> Result<(), ConnectError> {
    let body = Bytes::from(
        DeleteUserRequest {
            email: email.to_string(),
            ..Default::default()
...
async fn current_user_via_middleware(pool: &SqlitePool, admin_emails: &[String], header_email: &str, header_name: &str) -> CurrentUser {
    ...
    let app = axum::Router::new()
        .route(
            "/probe",
            axum::routing::get(move |axum::extract::Extension(current): axum::extract::Extension<CurrentUser>| {
        ...
        .layer(axum::middleware::from_fn_with_state(auth_state, auth::middleware));
    ...
    let response = tower::ServiceExt::oneshot(app, request).await.expect("drive middleware");
    assert_eq!(response.status(), http::StatusCode::OK, "probe handler must be reached");

    captured.lock().await.take().expect("middleware must have attached a CurrentUser")
}
...
async fn non_admin_caller_gets_permission_denied_from_every_rpc() {
    ...
    let err = delete_user(&svc, ctx_for("caller@example.com", vec![UserRole::Pm]), "someone@example.com")
        .await
...
async fn admin_caller_can_list_upsert_and_delete() {
    ...
    let listed = list_users(&svc, admin_ctx()).await.expect("admin list must succeed");
...
async fn upsert_with_empty_roles_is_invalid_argument_and_writes_nothing() {
    ...
    assert_eq!(user_row_count(&pool).await, rows_before, "rejected upsert must not write a row");
...
async fn pre_creation_then_first_login_carries_the_admin_assigned_roles() {
    ...
    let current = current_user_via_middleware(&pool, &[], "precreated@example.com", "Pre Created").await;
    ...
    assert_eq!(current.name, "Pre Created", "first login must still supply the real name");
...
async fn later_login_updates_name_but_leaves_roles_untouched() {
    ...
    let first = current_user_via_middleware(&pool, &[], "returning@example.com", "First Name").await;
    assert_eq!(first.name, "First Name");
    assert_eq!(first.roles, vec![UserRole::Bl]);

    let second = current_user_via_middleware(&pool, &[], "returning@example.com", "Updated Name").await;
    assert_eq!(second.name, "Updated Name", "a later login must refresh the name");
    assert_eq!(second.roles, vec![UserRole::Bl], "a later login must never touch admin-managed roles");
...
async fn ensure_admins_is_idempotent_and_does_not_clobber_existing_extra_roles() {
    ...
    auth::ensure_admins(&pool, &admin_emails).await.expect("ensure_admins ok");
    assert_eq!(stored_roles(&pool, "will-become-admin@example.com").await, vec![UserRole::Admin, UserRole::Pm]);

    // Idempotent: running it again must not duplicate or otherwise change
    // the outcome.
    auth::ensure_admins(&pool, &admin_emails).await.expect("ensure_admins ok (second call)");
    assert_eq!(stored_roles(&pool, "will-become-admin@example.com").await, vec![UserRole::Admin, UserRole::Pm]);
...
async fn ensure_admins_creates_a_placeholder_row_for_a_never_seen_email() {
    ...
    assert_eq!(stored_roles(&pool, "brand-new-admin@example.com").await, vec![UserRole::Admin]);
    let name: String = sqlx::query_scalar("SELECT name FROM users WHERE email = ?1")
        .bind("brand-new-admin@example.com")
        .fetch_one(&pool)
        .await
        .expect("fetch name");
    assert_eq!(name, "brand-new-admin@example.com", "placeholder name must be the email itself");
