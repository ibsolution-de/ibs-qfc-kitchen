

export const translations = {
  en: {
    roles: {
      employee: 'Employee',
      pm: 'Project Manager',
      bl: 'Division Manager (BL)',
      sales: 'Sales Manager',
      admin: 'Administrator'
    },
    common: {
      loading: 'Loading…',
      loadError: 'Failed to load data from the server. Please try again later.',
      saveError: 'Failed to save your change. Please try again.',
      sessionExpired: 'Your session has expired. Please sign in again.',
      retry: 'Retry',
      reload: 'Reload page'
    },
    accessibility: {
      chartData: 'Chart data',
      bin: 'Bin',
      count: 'Count',
    },
    myOverview: {
      title: 'My Overview',
      subtitle: 'Your schedule and tasks',
      myCapacity: 'My Capacity',
      myProjects: 'My Projects',
      reportStatus: 'Report Status',
      upcoming: 'Upcoming Days',
      daysPlaned: 'Days Planned',
      utilization: 'Utilization',
      statusReportTitle: 'Submit Weekly Status',
      mood: 'How do you feel?',
      blockers: 'Any blockers?',
      submit: 'Submit Report',
      feelingGood: 'Feeling Good',
      overwhelmed: 'Overwhelmed',
      bored: 'Bored',
      pulseCheck: 'Pulse Check',
      pulseCheckTitle: 'Pre-1:1 Pulse Check',
      pulseCheckDesc: 'Help your lead prepare for your upcoming 1:1 on',
      publicHoliday: 'Public Holiday',
      noProjectsAssigned: 'No projects assigned currently.',
      achievedPlaceholder: 'What did you achieve this week?',
      employeeOverview: "{{name}}'s Overview",
      assignedDays: '{{count}}d assigned',
      reportSent: 'Report Sent!',
      employeeNotFound: 'Employee not found.',
      unlinkedTitle: 'Account Not Linked',
      unlinkedBody: "Your account isn't linked to an employee record yet, so we can't show your schedule or stats.",
      unlinkedAdminHint: 'As an admin, you can link it under Administration → Users.',
      unlinkedTeamHint: 'Ask to have your email added to your employee record on the Team page — accounts are linked automatically by email.'
    },
    oneOnOne: {
        title: 'Smart 1:1 & Feedback',
        subtitle: 'Continuous feedback and coaching loops',
        scheduleTitle: 'Session Schedule',
        upcoming: 'Upcoming',
        past: 'History',
        notes: 'Session Notes',
        notesPlaceholder: 'Capture discussion points, feedback, and key takeaways...',
        elephantMemory: 'The Elephant Memory',
        elephantMemoryDesc: 'Commitments from previous sessions. Don\'t forget these!',
        noCommitments: 'No open commitments.',
        pulse: 'Sentiment Pulse',
        pulseGreat: 'Feeling Great',
        pulseOkay: 'Doing Okay',
        pulseStress: 'Stressed/Overwhelmed',
        pulseUnknown: 'Not submitted',
        aiTools: 'AI Coach',
        generateAgenda: 'Generate Coaching Agenda',
        extractActions: 'Extract Action Items',
        generating: 'Generating...',
        agenda: 'Suggested Agenda',
        actionItems: 'Extracted Commitments',
        emptyState: 'Select a session to view details or schedule a new one.',
        scheduleNew: 'Schedule New 1:1',
        markComplete: 'Mark as Completed',
        newSession: 'New Session',
        nextCheckIn: 'Next Check-in',
        inTwoWeeks: 'In 2 weeks',
        draft: 'Draft',
        noAgendaItems: 'No agenda items yet. Add topics to discuss.',
        addAgendaItem: 'Add agenda item...',
        addActionItem: 'Add action item...',
        voiceDictation: 'Voice Dictation (Simulated)',
        agendaItemsCount: '{{count}} agenda items'
    },
    planner: {
      month: 'Month',
      quarter: 'Quarter',
      current: 'Current',
      today: 'Today',
      currentMonth: 'Current Month',
      exportState: 'Export State',
      exportCSV: 'Export CSV',
      clientExport: 'Client Export',
      legend: 'Legend',
      projectKey: 'Project Key',
      employee: 'Employee',
      week: 'KW',
      days: 'Days',
      repeatMode: 'Repeat for entire month',
      applyOnDays: 'Apply on days',
      scopeDay: 'Single day',
      scopeMonth: 'Plan whole month',
      scopeDays: 'Several working days',
      selectAll: 'Select all',
      monthHint: 'Assigns the selected projects to these weekdays across the whole month.',
      selectProjects: 'Select Projects',
      searchProjects: 'Search projects...',
      all: 'All',
      oneClickAssign: 'Manage Day',
      assigned: 'Assigned',
      date: 'DATE',
      cancel: 'Cancel',
      save: 'Save Changes',
      clickToAssign: 'Click any project below to quickly assign it to all selected days.',
      plannedCapacity: 'Planned / Capacity',
      freeDays: 'Free Days',
      filterProject: 'Filter Project',
      allProjects: 'All Projects',
      filterActive: 'Active Projects',
      filterOpportunity: 'Opportunities',
      previousMonth: 'Previous month',
      nextMonth: 'Next month',
      allocation: 'Allocation',
      assignProject: 'Assign Project',
      markAbsence: 'Mark Absence',
      vacation: 'Vacation',
      sick: 'Sick Leave',
      training: 'Training',
      conflictAlert: 'Resource Conflict',
      criticalConflict: 'Multiple critical projects assigned!',
      overload: 'Overload',
      underutilized: 'Underutilized',
      holiday: 'Public Holiday',
      blocked: 'Blocked',
      tooltipCapacity: 'Planned Days / Total Capacity',
      tooltipFree: 'Net Available Capacity',
      tooltipOverload: 'Number of days with overload',
      conflictAbsence: 'This day already has an absence entry — it will be kept.',
      conflictAssignments: 'This day already has project assignments — they will be kept.',
      readOnly: 'Read Only',
      deleteAbsence: 'Delete absence',
      confirmDeleteTitle: 'Remove assignment',
      confirmDeleteMessage: 'Are you sure you want to remove this assignment?',
      confirmDeleteConfirm: 'Remove',
      gridLabel: 'Resource planning grid',
      absence: 'Absence',
      cellNoAssignments: 'No assignments',
      daysCount: 'Days count:',
      dailySchedule: 'Daily Schedule',
      total: 'Total',
      noProjectsAssigned: 'No projects assigned. Select below to add.',
      viewEmployeeOverview: 'View Employee Overview',
      remove: 'Remove',
      foreignContext: 'Planned by another PM',
      account: 'Account',
      selectAccount: 'Select account',
    },
    projects: {
      title: 'Projects',
      subtitle: 'Manage your projects portfolio',
      addProject: 'Add Project',
      isCritical: 'Critical Project',
      confirmDelete: 'Are you sure you want to delete this project?',
      editProject: 'Edit Project',
      newProject: 'New Project',
      projectName: 'Project Name',
      client: 'Client',
      topic: 'Topic',
      placeholderTopic: 'e.g. Cloud Migration',
      status: 'Status',
      budget: 'Budget',
      placeholderBudget: 'e.g. 50k',
      accounts: 'Accounts',
      addAccount: 'Add Account',
      accountName: 'Account Name',
      accountStatus: 'Status',
      statusConfirmed: 'Confirmed',
      statusRequested: 'Requested',
      accountStart: 'Start',
      accountEnd: 'End',
      accountBudget: 'Budget',
      estimatedBudget: 'Est. Budget',
      noAccounts: 'No accounts yet',
      volume: 'Volume (Days)',
      dates: 'Duration',
      colorTag: 'Color Tag',
      notes: 'Notes',
      saveProject: 'Save Project',
      cancel: 'Cancel',
      hourlyRate: 'Hourly Rate (€)',
      marginHealth: 'Margin Health',
      milestones: 'Milestones',
      addMilestone: 'Add Milestone',
      milestoneName: 'Milestone Name',
      phase: 'Phase',
      probability: 'Win Probability',
      salesStage: 'Pipeline Stage',
      delete: 'Delete',
      deleteTitle: 'Delete Project',
      validation: {
        dates: 'End date must be on or after the start date.',
        required: 'Project name and client are required.'
      }
    },
    sidebar: {
      myOverview: 'My Overview',
      resourcePlan: 'Resource Plan',
      quarterlyForecast: 'Quarterly Forecast',
      team: 'Team',
      projects: 'Projects',
      customers: 'Customers',
      financials: 'Financials',
      strategy: 'Strategy & Vision',
      salesPipeline: 'Sales Pipeline',
      planning: 'PLANNING',
      manage: 'Master Data',
      versions: 'Plan Revisions',
      settings: 'Settings',
      language: 'Language',
      aiConfig: 'AI Config',
      enableAI: 'Enable AI Features',
      apiKey: 'AI API Key',
      apiKeyPlaceholder: 'Enter your API Key...',
      getKey: 'Get API Key',
      close: 'Close',
      saveSettings: 'Save Settings',
      switchRole: 'Switch Role',
      clearRoleOverride: 'Reset to my real roles',
      saveNewVersion: 'Freeze new plan revision',
      defaultPlanName: 'Resource Plan',
      clearApiKey: 'Clear API Key',
      latest: 'Latest',
      frozen: 'Frozen',
      changelogTitle: 'System Record',
      administration: 'Administration',
      userAdministration: 'Users & System'
    },
    changelog: {
      title: 'CHANGELOG.SYS',
      systemOnline: 'SYSTEM ONLINE',
      id: 'ID: QFC-KITCHEN-CORE',
      v140: {
        title: 'BACKEND & MULTI-USER',
        serverStorage: 'Shared Server Storage',
        serverStorageDesc: 'Plans, teams, customers and forecasts now live in a server database instead of a single browser — everyone works on the same data.',
        liveUpdates: 'Live Updates',
        liveUpdatesDesc: 'Changes made by others show up without a reload; the app subscribes to a server-push event stream.',
        signIn: 'Real Sign-In Identity',
        signInDesc: 'Your account and roles come from the company sign-in instead of a hardcoded demo user.',
        multiRole: 'Multiple Roles per User',
        multiRoleDesc: 'One user can hold employee, PM, BL, sales and admin at once; the landing page follows the highest role.',
        adminArea: 'Admin Area',
        adminAreaDesc: 'Admins manage users and roles, application defaults, and watch live system status under Administration.'
      },
      v132: {
        title: 'DEPLOYMENT & INFRASTRUCTURE',
        containerized: 'Containerized Delivery',
        containerizedDesc: 'The app now ships as a self-contained static nginx image, built automatically from source.',
        hardened: 'Hardened Kubernetes Runtime',
        hardenedDesc: 'Runs non-root with a read-only filesystem under restricted Pod Security — production-ready by default.',
        gitops: 'Automated GitOps Deploy',
        gitopsDesc: 'Every push builds an immutable image and ArgoCD syncs the cluster to match — no manual steps.'
      },
      v131: {
        title: 'STABILITY & PERFORMANCE UPDATE',
        dataSafety: 'Planner Data Safety',
        dataSafetyDesc: 'Saving assignments or absences now merges entries — no more silent loss of the other type. Includes conflict warnings.',
        overload: 'Availability-Aware Overloads',
        overloadDesc: 'Overload detection respects part-time availability — cell and utilization always agree.',
        exports: 'Real Exports & Customer CRUD',
        exportsDesc: 'CSV plan export, JSON forecast export, and full customer management replace dead buttons.',
        performance: 'Massive Performance Boost',
        performanceDesc: 'Initial bundle 909 → 97 kB, planner grid indexed and memoized, forecast inputs commit on blur.',
        quality: 'Quality Foundation',
        qualityDesc: 'Strict TypeScript, 84 automated tests, accessible modals, full German/English coverage.'
      },
      salesModuleActivated: 'SALES MODULE ACTIVATED',
      salesPipelineDashboard: 'Sales Pipeline Dashboard',
      salesPipelineDesc: 'Kanban-style workflow for managing Leads, Opportunities, and Negotiations. Integrated probability tracking.',
      marketTrendsScout: 'Market Trends Scout (AI)',
      marketTrendsDesc: 'New AI capability to scan for industry trends and generate potential leads.',
      rootPrompt: 'root@ibs-qfc:~$'
    },
    forecast: {
      title: 'Quarterly Forecast',
      subtitle: 'Manage capacity planning and opportunities',
      current: 'Current',
      teamCapacity: 'Team Capacity',
      runningProjects: 'Running Projects',
      available: 'Available',
      monthlySplit: 'Monthly Breakdown',
      metric: 'Metric',
      totalCapacity: 'Total Capacity',
      optimistic: 'Optimistic',
      mustWin: 'Must-Win',
      alternatives: 'Alternatives',
      netCapacity: 'Net Capacity',
      overcapacity: 'Overcapacity',
      notes: 'Notes',
      notesEmpty: 'No notes yet.',
      notesPlaceholder: 'Notes for this quarter...',
      notesSaveHint: 'Saved automatically when you leave the field (Ctrl/⌘+Enter to save without leaving).',
      remove: 'Remove',
      volumeDays: 'Volume (Days)',
      start: 'Start',
      budgetPlaceholder: 'Budget',
      noneIdentified: 'None identified',
      addOpp: 'Add Opportunity',
      addAlt: 'Add Alternative',
      selectProject: 'Select Project',
      noMoreOpp: 'No more opportunities available',
      noActive: 'No active projects',
      noQuarters: 'No quarters to show yet.',
      noData: 'No forecast data available.',
      exportJSON: 'Export JSON',
      aiAnalysis: 'AI Analysis',
      aiModel: 'AI Model',
      processingVia: 'Processing via {{model}}',
      analysisTitle: 'Strategic Analysis',
      generatingAnalysis: 'Generating analysis...',
      analysisError: 'Failed to generate analysis.',
      aiSetupTitle: 'Setup AI Insights',
      aiSetupBody: 'Unlock strategic insights for your quarterly forecast using Gemini AI. Please enable AI features in the application settings and provide your own API Key.',
      aiPrivacyTitle: 'Privacy & Security',
      aiPrivacyBody: 'Your API Key is stored locally in your browser\'s storage. It connects directly to Google\'s API and is never transmitted to or stored on our servers.',
      openSettingsInstruction: 'Navigate to Settings (bottom left sidebar) to configure AI access.',
      configureSettings: 'Configure Settings',
      monteCarlo: 'Monte Carlo Simulation',
      runSim: 'Run Sim',
      probability: 'Prob.',
      simTitle: 'Opportunity Simulation',
      simSubtitle: 'Probability-weighted capacity analysis (2000 iterations)',
      p90: 'Conservative (P90)',
      p50: 'Median (P50)',
      p10: 'Aggressive (P10)',
      simScenarioHigh: 'High Load Scenario',
      simScenarioMed: 'Likely Outcome',
      simScenarioLow: 'Low Load Scenario',
      netCap: 'Net Capacity',
      simDistribution: 'Probability Distribution',
      riskOfOverload: 'Risk of Overload',
      capacityLimit: 'Capacity Limit',
      safeZone: 'Safe Zone',
      overloadZone: 'Overload Zone',
    },
    version: {
      title: 'Create New Plan Revision',
      name: 'Plan Revision Name',
      placeholderName: 'e.g. Adjusted Q4 Plan',
      description: 'Description',
      placeholderDesc: 'What changed in this revision?',
      cancel: 'Cancel',
      create: 'Create Revision',
      copyHint: 'The new revision starts as a copy of the latest plan revision. Frozen revisions are read-only — new work only ever branches from the latest one.'
    },
    versions: {
      rename: 'Rename',
      delete: 'Delete',
      deleteTitle: 'Delete Plan Revision',
      deleteMessage: 'Are you sure you want to delete plan revision "{{name}}"?',
      compare: 'Compare',
      compareSelect: 'Select base and target revisions',
      diffAdded: 'Added',
      diffRemoved: 'Removed',
      diffChanged: 'Changed',
      diffEmpty: 'No differences found.',
      toastRenamed: 'Plan revision renamed successfully.',
      toastDeleted: 'Plan revision deleted successfully.',
      lastVersionGuard: 'Cannot delete the last plan revision.',
      cancel: 'Cancel'
    },
    team: {
      title: 'Team Management',
      subtitle: 'Manage employees, extended team, and hiring',
      addEmployee: 'Add Employee',
      addFutureHire: 'Plan Future Hire',
      confirmDelete: 'Are you sure you want to delete this employee?',
      editEmployee: 'Edit Employee',
      uploadPhoto: 'Click to upload',
      name: 'Name',
      placeholderName: 'Full Name',
      role: 'Role',
      placeholderRole: 'e.g. Senior Developer',
      email: 'Email',
      placeholderEmail: 'email@company.com',
      phone: 'Phone',
      placeholderPhone: '+49 ...',
      availability: 'Availability',
      location: 'Location (Holiday Calendar)',
      skills: 'Skills',
      typeSkill: 'Type skill and press Enter...',
      notes: 'Notes',
      placeholderNotes: 'Additional information...',
      saveEmployee: 'Save Employee',
      cancel: 'Cancel',
      noSkills: 'No skills listed',
      coreTeam: 'Core Team',
      extendedTeam: 'Extended Team / Other Divisions',
      futureHiring: 'Future Hiring & Planning',
      smartRecruit: 'Smart Recruit',
      smartRecruitSubtitle: 'Internal suggestions based on resource gaps',
      addToTeam: 'Add to Extended Team',
      matchScore: 'Match',
      noSuggestions: 'No suitable candidates found in directory.',
      open1on1: 'Open 1:1 Dashboard',
      openDevelopment: 'Development & Positioning',
      skillMatrix: 'Skill Matrix',
      directory: 'Directory',
      delete: 'Delete',
      deleteTitle: 'Delete Employee',
      validation: {
        required: 'Name and role are required.'
      }
    },
    admin: {
      title: 'User Administration',
      subtitle: 'Manage accounts and role assignments',
      addUser: 'Add User',
      editUser: 'Edit User',
      deleteUser: 'Delete User',
      email: 'Email',
      emailPlaceholder: 'name@company.com',
      roles: 'Roles',
      linkedEmployee: 'Linked Employee',
      noEmployeeLink: 'No employee link',
      cancel: 'Cancel',
      saveUser: 'Save User',
      deleteTitle: 'Delete User',
      confirmDelete: 'Are you sure you want to delete this user? This cannot be undone.',
      delete: 'Delete',
      noUsers: 'No users yet.',
      retry: 'Retry',
      validation: {
        emailRequired: 'Email is required.',
        roleRequired: 'At least one role is required.'
      },
      errorInvalidRoles: 'This user needs at least one role.',
      errorDeleteGuard: 'Cannot delete the last admin or your own account.',
      errorUserNotFound: 'This user no longer exists.',
      errorPermissionDenied: 'You do not have permission to manage users.',
      tabs: {
        users: 'Users & Roles',
        setup: 'Application Setup',
        monitoring: 'System & KPIs'
      },
      setup: {
        title: 'Application Setup',
        subtitle: 'Runtime-editable application settings',
        defaultRoleLabel: 'Default Role',
        adminEmailsLabel: 'Admin Emails',
        adminEmailsPlaceholder: 'name@company.com',
        planRevisionRetentionLabel: 'Keep plan revisions',
        planRevisionRetentionHint: 'How many frozen planning snapshots (plan revisions) stay saved. When a new revision is created — manually or automatically each quarter — older ones beyond this number are removed.',
        environmentLabel: 'Environment',
        environmentEmpty: 'none configured',
        overrideBadge: 'DB Override',
        firstSeenHint: 'Changes only affect users the server sees for the first time afterwards. Existing accounts keep their current roles.',
        save: 'Save Settings',
        saving: 'Saving…',
        validationInvalidEmail: 'Every line must be a valid email address.',
        errorInvalidSettings: 'The server rejected these settings. Please check the values.'
      },
      monitoring: {
        title: 'System & KPIs',
        subtitle: 'Server status and database metrics',
        uptime: 'Uptime',
        version: 'Version',
        dbSize: 'Database Size',
        liveClients: 'Live Clients',
        devModeWarning: 'Dev-user mode is active (QFC_DEV_USER) - never enable this in production.',
        entitiesTitle: 'Entities',
        entities: {
          users: 'Users',
          employees: 'Employees',
          customers: 'Customers',
          projects: 'Projects',
          planVersions: 'Plan Versions',
          assignments: 'Assignments',
          absences: 'Absences',
          quarterData: 'Forecast Quarters',
          strategicGoals: 'Strategic Goals',
          northStarMetrics: 'North Star Metrics',
          oneOnOneSessions: '1:1 Sessions',
          publicHolidays: 'Public Holidays'
        },
        changeLogTitle: 'Change Log',
        changeLogRows: 'Rows',
        changeLogSeqRange: 'Sequence Range',
        changeLogRetention: 'Retention Cap',
        lastUpdated: 'Last updated',
        autoRefresh: 'Auto-refresh'
      }
    },
    development: {
      title: 'Positioning & Development',
      ikigai: 'My Ikigai',
      competencies: 'Wheel of Competencies',
      dragDropInstructions: 'Drag items to their correct zone.',
      addActivity: 'Add Activity',
      zones: {
        love: 'Love it',
        good: 'Good at it',
        paid: 'Paid for it',
        needed: 'World needs it'
      },
      gapAnalysis: 'Gap Analysis',
      self: 'Self',
      manager: 'Manager',
      projectSimulator: 'Project Simulator',
      simulatorDesc: 'Check team readiness for a potential project.',
      requiredSkill: 'Required Skill',
      requiredLevel: 'Required Level (1-5)',
      checkReadiness: 'Check Readiness',
      ready: 'Ready',
      gap: 'Gap',
      qualified: 'Qualified',
      skillGap: 'Skill Gap'
    },
    customers: {
      title: 'Customer Overview',
      subtitle: 'Track customer portfolio and health',
      addCustomer: 'Add Customer',
      newCustomer: 'New Customer',
      editCustomer: 'Edit Customer',
      deleteCustomer: 'Delete Customer',
      customerName: 'Customer Name',
      logo: 'Logo URL',
      industry: 'Industry',
      contactName: 'Contact Name',
      email: 'Email',
      notes: 'Notes',
      saveCustomer: 'Save Customer',
      cancel: 'Cancel',
      confirmDelete: 'Are you sure you want to delete this customer?',
      hrCoverage: 'HR Coverage',
      days: 'Days',
      budgetCoverage: 'Budget Coverage',
      projects: 'Active Projects',
      planned: 'Planned',
      accountsCount: '{{count}} accounts',
      gap: 'Gap',
      onTrack: 'On Track',
      noProjects: 'No active projects for this customer.',
      delete: 'Delete',
      deleteTitle: 'Delete Customer',
      validation: {
        required: 'Customer name and email are required.'
      }
    },
    financials: {
        title: 'Financial Overview',
        subtitle: 'Revenue forecasts and project profitability',
        revenueForecast: 'Revenue Forecast (4 Quarters)',
        profitability: 'Project Profitability & Margins',
        project: 'Project',
        budget: 'Budget',
        plannedCost: 'Planned Cost',
        margin: 'Margin',
        status: 'Status',
        all: 'All',
        highRisk: 'High Risk',
        healthy: 'Healthy',
        low: 'Low',
        lowMargin: 'Low Margin',
        risk: 'Risk',
        drawable: 'Drawable',
        overrun: 'Overrun',
        account: 'Account',
        totalRevenue: 'Total Revenue',
        avgMargin: 'Avg Margin',
        noProjectsFound: 'No projects found matching your criteria.',
        noProjects: 'No projects yet. Add a project to see financials here.',
        noBudgetsSet: 'None of your projects have a budget set. Add a budget to a project to see its profitability.',
        noProjectDates: 'Add start and end dates to your projects to see them reflected in the revenue forecast.',
        revenueNoData: 'No revenue forecast data yet.',
        revenueNoDataHint: 'Revenue is distributed from project budgets over their start–end period. Add a budget and start/end dates to at least one project, and the quarterly chart appears here.'
    },
    strategy: {
        title: 'Strategy & Vision',
        subtitle: 'Align projects with company goals and north star metrics',
        strategyMap: 'Strategy Map (Balanced Scorecard)',
        northStar: 'North Star Alignment',
        aiCoPilot: 'AI Strategy Co-Pilot',
        perspectives: {
            financial: 'Financial',
            customer: 'Customer',
            internal: 'Internal Process',
            learning: 'Learning & Growth'
        },
        addGoal: 'Add Goal',
        newGoal: 'New Goal',
        goalTitle: 'Goal Title',
        goalPerspective: 'Perspective',
        saveGoal: 'Save Goal',
        cancel: 'Cancel',
        sunburstTitle: 'Work Volume vs Strategic Metrics',
        sunburstSubtitle: 'Inner Ring: Metric Categories | Outer Ring: Contributing Projects',
        totalVolume: 'Total Volume',
        maintenance: 'Maintenance & Other',
        coPilotTitle: 'Strategy Consultant',
        coPilotSubtitle: 'I will help you draft your strategy document.',
        startInterview: 'Start Strategy Interview',
        generateDoc: 'Generate Strategy Doc',
        interviewMode: 'INTERVIEW MODE',
        docMode: 'DOCUMENT MODE',
        aiDisabled: 'Please enable AI in settings and provide an API Key.',
        aiError: 'Error connecting to AI Consultant.',
        greeting: 'I will help you draft your strategy document. Start Strategy Interview?',
        configureAi: 'Configure AI'
    },
    sales: {
        title: 'Sales Pipeline',
        subtitle: 'Manage leads and opportunities flow',
        addLead: 'Add Lead',
        marketTrends: 'Market Trends',
        pipelineValue: 'Pipeline Value',
        weightedValue: 'Weighted Value',
        activeLeads: 'Active Leads',
        stages: {
            lead: 'New Lead',
            qualified: 'Qualified',
            proposal: 'Proposal',
            negotiation: 'Negotiation'
        },
        marketScoutTitle: 'Market Trends Scout',
        marketScoutSubtitle: 'Use AI to identify potential projects',
        trendsPrompt: 'What industry or client should we analyze?',
        analyze: 'Analyze Trends',
        analyzing: 'Scouting market data...',
        aiNotConfigured: 'AI Not Configured. Check Settings.',
        aiErrorTrends: 'Error analyzing trends. Check API Key.',
        placeholderProjectName: 'e.g. Website Relaunch',
        placeholderClient: 'Client Name'
    },
    chat: {
      assistantReady: 'ASSISTANT_READY',
      resourceAssistant: 'Resource_Assistant',
      howCanIHelp: 'How can I help with resource planning?',
      askAbout: 'Ask about capacity, conflicts, or specific projects.',
      accessDenied: 'ACCESS_DENIED: AI Module not configured. Please set API Key in settings.',
      errorComputation: 'ERROR: Computation failed. Please retry.',
      sendError: 'Failed to send message. Please check your connection and try again.',
      inputPlaceholder: 'Ask about resources, projects or conflicts...',
      aiConfigMissing: 'AI Configuration Missing',
      enableInSettings: 'Enable in Settings',
      you: 'You',
      assistant: 'Assistant',
      suggestion1: 'Show overallocated employees',
      suggestion2: 'List projects ending in Q4',
      suggestion3: 'Who has free capacity next month?',
      suggestion4: 'Are there any resource conflicts?',
      settings: 'Settings',
      minimize: 'Minimize',
      close: 'Close',
      restore: 'Restore chat',
      triggerLabel: 'Open resource assistant',
      send: 'Send message',
      confirmCloseTitle: 'Close Assistant',
      confirmCloseMessage: 'Closing the assistant will clear the current conversation. Do you want to continue?',
      confirmCloseConfirm: 'Close',
      confirmCloseCancel: 'Cancel'
    },
    status: {
      active: 'Active',
      opportunity: 'Opportunity',
      completed: 'Completed',
      on_hold: 'On Hold'
    },
    toast: {
      employeeSaved: 'Employee saved successfully.',
      employeeDeleted: 'Employee deleted successfully.',
      projectSaved: 'Project saved successfully.',
      projectDeleted: 'Project deleted successfully.',
      customerSaved: 'Customer saved successfully.',
      customerDeleted: 'Customer deleted successfully.',
      versionCreated: 'New plan revision created successfully.',
      planCreated: 'Resource plan created',
      csvExported: 'CSV exported successfully.',
      userSaved: 'User saved successfully.',
      userDeleted: 'User deleted successfully.',
      settingsSaved: 'Settings saved successfully.'
    }
  },
  de: {
    roles: {
      employee: 'Mitarbeiter',
      pm: 'Projektleiter (PM)',
      bl: 'Bereichsleiter (BL)',
      sales: 'Sales Manager',
      admin: 'Administrator'
    },
    common: {
      loading: 'Laden…',
      loadError: 'Daten konnten nicht vom Server geladen werden. Bitte später erneut versuchen.',
      saveError: 'Änderung konnte nicht gespeichert werden. Bitte erneut versuchen.',
      sessionExpired: 'Ihre Sitzung ist abgelaufen. Bitte melden Sie sich erneut an.',
      retry: 'Erneut versuchen',
      reload: 'Seite neu laden'
    },
    accessibility: {
      chartData: 'Diagrammdaten',
      bin: 'Bin',
      count: 'Anzahl',
    },
    myOverview: {
      title: 'Meine Übersicht',
      subtitle: 'Dein Zeitplan und Aufgaben',
      myCapacity: 'Meine Kapazität',
      myProjects: 'Meine Projekte',
      reportStatus: 'Status melden',
      upcoming: 'Kommende Tage',
      daysPlaned: 'Tage geplant',
      utilization: 'Auslastung',
      statusReportTitle: 'Wochenstatus senden',
      mood: 'Wie fühlst du dich?',
      blockers: 'Gibt es Blocker?',
      submit: 'Bericht senden',
      feelingGood: 'Mir gehts gut',
      overwhelmed: 'Überlastet',
      bored: 'Gelangweilt',
      pulseCheck: 'Stimmungscheck',
      pulseCheckTitle: '1:1 Vorbereitung',
      pulseCheckDesc: 'Hilf deinem Lead bei der Vorbereitung auf das 1:1 am',
      publicHoliday: 'Feiertag',
      noProjectsAssigned: 'Derzeit keine Projekte zugewiesen.',
      achievedPlaceholder: 'Was hast du diese Woche erreicht?',
      employeeOverview: 'Übersicht von {{name}}',
      assignedDays: '{{count}}d zugewiesen',
      reportSent: 'Bericht gesendet!',
      employeeNotFound: 'Mitarbeiter nicht gefunden.',
      unlinkedTitle: 'Konto nicht verknüpft',
      unlinkedBody: 'Dein Konto ist noch keinem Mitarbeiterdatensatz zugeordnet, daher können wir deinen Zeitplan und deine Statistiken nicht anzeigen.',
      unlinkedAdminHint: 'Als Administrator kannst du dies unter Verwaltung → Benutzer verknüpfen.',
      unlinkedTeamHint: 'Bitte lass deine E-Mail-Adresse im Mitarbeiterdatensatz auf der Team-Seite hinterlegen — die Verknüpfung erfolgt automatisch per E-Mail.'
    },
    oneOnOne: {
        title: 'Smart 1:1 & Feedback',
        subtitle: 'Kontinuierliches Feedback und Coaching',
        scheduleTitle: 'Terminplan',
        upcoming: 'Kommend',
        past: 'Historie',
        notes: 'Sitzungsnotizen',
        notesPlaceholder: 'Diskussionspunkte, Feedback und Erkenntnisse festhalten...',
        elephantMemory: 'Das Elefantengedächtnis',
        elephantMemoryDesc: 'Zusagen aus vorherigen Sitzungen. Nicht vergessen!',
        noCommitments: 'Keine offenen Zusagen.',
        pulse: 'Stimmungsbild',
        pulseGreat: 'Fühlt sich gut',
        pulseOkay: 'Geht so',
        pulseStress: 'Gestresst/Überlastet',
        pulseUnknown: 'Nicht eingereicht',
        aiTools: 'KI Coach',
        generateAgenda: 'Agenda generieren',
        extractActions: 'Aufgaben extrahieren',
        generating: 'Generiere...',
        agenda: 'Vorgeschlagene Agenda',
        actionItems: 'Extrahierte Zusagen',
        emptyState: 'Wähle eine Sitzung aus oder plane eine neue.',
        scheduleNew: 'Neues 1:1 planen',
        markComplete: 'Als erledigt markieren',
        newSession: 'Neue Sitzung',
        nextCheckIn: 'Nächstes Check-in',
        inTwoWeeks: 'In 2 Wochen',
        draft: 'Entwurf',
        noAgendaItems: 'Noch keine Agenda-Punkte. Fügen Sie Themen hinzu.',
        addAgendaItem: 'Agenda-Punkt hinzufügen...',
        addActionItem: 'Aufgabe hinzufügen...',
        voiceDictation: 'Spracheingabe (Simuliert)',
        agendaItemsCount: '{{count}} Agenda-Punkte'
    },
    planner: {
      month: 'Monat',
      quarter: 'Quartal',
      current: 'Aktuell',
      today: 'Heute',
      currentMonth: 'Aktueller Monat',
      exportState: 'Status exportieren',
      exportCSV: 'CSV Export',
      clientExport: 'Kundenexport',
      legend: 'Legende',
      projectKey: 'Projektlegende',
      employee: 'Mitarbeiter',
      week: 'KW',
      days: 'Tage',
      repeatMode: 'Für ganzen Monat wiederholen',
      applyOnDays: 'An Tagen anwenden',
      scopeDay: 'Einzelner Tag',
      scopeMonth: 'Ganzen Monat verplanen',
      scopeDays: 'Mehrere Arbeitstage',
      selectAll: 'Alle auswählen',
      monthHint: 'Plant die ausgewählten Projekte für diese Wochentage über den ganzen Monat.',
      selectProjects: 'Projekte auswählen',
      searchProjects: 'Projekte suchen...',
      all: 'Alle',
      oneClickAssign: 'Tag verwalten',
      assigned: 'Zugewiesen',
      date: 'DATUM',
      cancel: 'Abbrechen',
      save: 'Speichern',
      clickToAssign: 'Klicken Sie auf ein Projekt für Sofortzuweisung an gewählten Tagen.',
      plannedCapacity: 'Geplant / Kapazität',
      freeDays: 'Freie Tage',
      filterProject: 'Projekt filtern',
      allProjects: 'Alle Projekte',
      filterActive: 'Aktive Projekte',
      filterOpportunity: 'Chancen',
      previousMonth: 'Vorheriger Monat',
      nextMonth: 'Nächster Monat',
      allocation: 'Auslastung',
      assignProject: 'Projekt zuweisen',
      markAbsence: 'Abwesenheit',
      vacation: 'Urlaub',
      sick: 'Krankheit',
      training: 'Schulung',
      conflictAlert: 'Ressourcenkonflikt',
      criticalConflict: 'Mehrere kritische Projekte zugewiesen!',
      overload: 'Überlast',
      underutilized: 'Unterausgelastet',
      holiday: 'Feiertag',
      blocked: 'Blockiert',
      tooltipCapacity: 'Geplante Tage / Gesamtkapazität',
      tooltipFree: 'Netto verfügbare Kapazität',
      tooltipOverload: 'Anzahl der Tage mit Überlast',
      conflictAbsence: 'An diesem Tag ist bereits eine Abwesenheit eingetragen — sie bleibt erhalten.',
      conflictAssignments: 'An diesem Tag sind bereits Projekte zugewiesen — sie bleiben erhalten.',
      readOnly: 'Nur Lesen',
      deleteAbsence: 'Abwesenheit löschen',
      confirmDeleteTitle: 'Zuweisung entfernen',
      confirmDeleteMessage: 'Möchten Sie diese Zuweisung wirklich entfernen?',
      confirmDeleteConfirm: 'Entfernen',
      gridLabel: 'Ressourcenplanungsraster',
      absence: 'Abwesenheit',
      cellNoAssignments: 'Keine Zuweisungen',
      daysCount: 'Anzahl Tage:',
      dailySchedule: 'Tagesplanung',
      total: 'Gesamt',
      noProjectsAssigned: 'Keine Projekte zugewiesen. Wählen Sie unten aus.',
      viewEmployeeOverview: 'Mitarbeiterübersicht anzeigen',
      remove: 'Entfernen',
      foreignContext: 'Von anderem PM geplant',
      account: 'Auftrag',
      selectAccount: 'Auftrag wählen',
    },
    projects: {
      title: 'Projektverwaltung',
      subtitle: 'Projekte und Möglichkeiten verwalten',
      addProject: 'Projekt hinzufügen',
      isCritical: 'Kritisches Projekt',
      confirmDelete: 'Möchten Sie dieses Projekt wirklich löschen?',
      editProject: 'Projekt bearbeiten',
      newProject: 'Neues Projekt',
      projectName: 'Projektname',
      client: 'Kunde',
      topic: 'Thema',
      placeholderTopic: 'z.B. Cloud Migration',
      status: 'Status',
      budget: 'Budget',
      placeholderBudget: 'z.B. 50k',
      accounts: 'Beauftragungen',
      addAccount: 'Beauftragung hinzufügen',
      accountName: 'Bezeichnung',
      accountStatus: 'Status',
      statusConfirmed: 'Eingegangen',
      statusRequested: 'In Anfrage',
      accountStart: 'Start',
      accountEnd: 'Ende',
      accountBudget: 'Budget',
      estimatedBudget: 'Budget (Schätzung)',
      noAccounts: 'Noch keine Beauftragungen',
      volume: 'Volumen (Tage)',
      dates: 'Laufzeit',
      colorTag: 'Farbmarkierung',
      notes: 'Notizen',
      saveProject: 'Projekt speichern',
      cancel: 'Abbrechen',
      hourlyRate: 'Stundensatz (€)',
      marginHealth: 'Margen Gesundheit',
      milestones: 'Meilensteine',
      addMilestone: 'Meilenstein hinzufügen',
      milestoneName: 'Name',
      phase: 'Phase',
      probability: 'Gewinnwahrscheinlichkeit',
      salesStage: 'Pipeline Phase',
      delete: 'Löschen',
      deleteTitle: 'Projekt löschen',
      validation: {
        dates: 'Enddatum muss am oder nach dem Startdatum liegen.',
        required: 'Projektname und Kunde sind Pflichtfelder.'
      }
    },
    sidebar: {
      myOverview: 'Meine Übersicht',
      resourcePlan: 'Ressourcenplaner',
      quarterlyForecast: 'Quartalsplanung',
      team: 'Team',
      projects: 'Projekte',
      customers: 'Kunden',
      financials: 'Finanzen',
      strategy: 'Strategie & Vision',
      salesPipeline: 'Sales Pipeline',
      planning: 'PLANUNG',
      manage: 'Stammdaten',
      versions: 'Plan-Revisionen',
      settings: 'Einstellungen',
      language: 'Sprache',
      aiConfig: 'KI Konfiguration',
      enableAI: 'KI-Funktionen aktivieren',
      apiKey: 'AI API Key',
      apiKeyPlaceholder: 'API Key eingeben...',
      getKey: 'Key anfordern',
      close: 'Schließen',
      saveSettings: 'Einstellungen speichern',
      switchRole: 'Rolle wechseln',
      clearRoleOverride: 'Auf meine echten Rollen zurücksetzen',
      saveNewVersion: 'Neue Plan-Revision einfrieren',
      defaultPlanName: 'Resourceplan',
      clearApiKey: 'API-Key löschen',
      latest: 'Neueste',
      frozen: 'Eingefroren',
      changelogTitle: 'Systemprotokoll',
      administration: 'Administration',
      userAdministration: 'Benutzer & System'
    },
    changelog: {
      title: 'CHANGELOG.SYS',
      systemOnline: 'SYSTEM ONLINE',
      id: 'ID: QFC-KITCHEN-CORE',
      v140: {
        title: 'BACKEND & MEHRBENUTZER',
        serverStorage: 'Gemeinsame Server-Speicherung',
        serverStorageDesc: 'Pläne, Teams, Kunden und Forecasts liegen jetzt in einer Server-Datenbank statt in einem einzelnen Browser — alle arbeiten auf denselben Daten.',
        liveUpdates: 'Live-Aktualisierung',
        liveUpdatesDesc: 'Änderungen anderer erscheinen ohne Neuladen; die App abonniert einen Server-Push-Eventstream.',
        signIn: 'Echte Anmelde-Identität',
        signInDesc: 'Dein Konto und deine Rollen kommen aus der Firmenanmeldung statt von einem fest verdrahteten Demo-Benutzer.',
        multiRole: 'Mehrere Rollen pro Benutzer',
        multiRoleDesc: 'Ein Benutzer kann Mitarbeiter, PM, BL, Sales und Admin gleichzeitig haben; die Startseite folgt der höchsten Rolle.',
        adminArea: 'Administrationsbereich',
        adminAreaDesc: 'Admins verwalten Benutzer und Rollen, Anwendungs-Standards und sehen den Live-Systemstatus unter Administration.'
      },
      v132: {
        title: 'DEPLOYMENT & INFRASTRUKTUR',
        containerized: 'Containerisierte Auslieferung',
        containerizedDesc: 'Die App wird jetzt als eigenständiges statisches nginx-Image ausgeliefert, automatisch aus dem Quellcode gebaut.',
        hardened: 'Gehärtete Kubernetes-Laufzeit',
        hardenedDesc: 'Läuft non-root mit read-only Dateisystem unter restricted Pod Security — standardmäßig produktionsbereit.',
        gitops: 'Automatisiertes GitOps-Deploy',
        gitopsDesc: 'Jeder Push baut ein unveränderliches Image, ArgoCD synchronisiert den Cluster automatisch — keine manuellen Schritte.'
      },
      v131: {
        title: 'STABILITÄTS- & PERFORMANCE-UPDATE',
        dataSafety: 'Planer-Datensicherheit',
        dataSafetyDesc: 'Speichern von Zuweisungen oder Abwesenheiten führt jetzt zusammen — kein stilles Löschen des anderen Typs mehr. Inkl. Konfliktwarnungen.',
        overload: 'Verfügbarkeitsbasierte Überlastung',
        overloadDesc: 'Überlastungserkennung berücksichtigt Teilzeit — Zelle und Auslastung stimmen immer überein.',
        exports: 'Echte Exporte & Kundenverwaltung',
        exportsDesc: 'CSV-Planexport, JSON-Forecast-Export und vollständige Kundenverwaltung ersetzen tote Buttons.',
        performance: 'Massiver Performance-Schub',
        performanceDesc: 'Initiales Bundle 909 → 97 kB, Planer-Grid indiziert und memoized, Forecast-Eingaben committen bei Blur.',
        quality: 'Qualitätsfundament',
        qualityDesc: 'Striktes TypeScript, 84 automatisierte Tests, barrierefreie Modals, vollständige DE/EN-Abdeckung.'
      },
      salesModuleActivated: 'SALES MODUL AKTIVIERT',
      salesPipelineDashboard: 'Sales Pipeline Dashboard',
      salesPipelineDesc: 'Kanban-Workflow für Leads, Chancen und Verhandlungen. Integrierte Wahrscheinlichkeitsverfolgung.',
      marketTrendsScout: 'Market Trends Scout (KI)',
      marketTrendsDesc: 'Neue KI-Funktion zur Branchentrenderkennung und Lead-Generierung.',
      rootPrompt: 'root@ibs-qfc:~$'
    },
    forecast: {
      title: 'Quartalsvorschau',
      subtitle: 'Kapazitätsplanung und Chancen verwalten',
      current: 'Aktuell',
      teamCapacity: 'Teamkapazität',
      runningProjects: 'Laufende Projekte',
      available: 'Verfügbar',
      monthlySplit: 'Monatliche Aufteilung',
      metric: 'Metrik',
      totalCapacity: 'Gesamtkapazität',
      optimistic: 'Optimistisch',
      mustWin: 'Must-Win',
      alternatives: 'Alternativen',
      netCapacity: 'Netto-Kapazität',
      overcapacity: 'Überkapazität',
      notes: 'Notizen',
      notesEmpty: 'Noch keine Notizen.',
      notesPlaceholder: 'Notizen zu diesem Quartal...',
      notesSaveHint: 'Wird automatisch gespeichert, wenn Sie das Feld verlassen (Strg/⌘+Enter speichert ohne zu verlassen).',
      remove: 'Entfernen',
      volumeDays: 'Volumen (Tage)',
      start: 'Start',
      budgetPlaceholder: 'Budget',
      noneIdentified: 'Keine identifiziert',
      addOpp: 'Chance hinzufügen',
      addAlt: 'Alternative hinzufügen',
      selectProject: 'Projekt wählen',
      noMoreOpp: 'Keine weiteren Chancen verfügbar',
      noActive: 'Keine aktiven Projekte',
      noQuarters: 'Noch keine Quartale vorhanden.',
      noData: 'Keine Prognosedaten verfügbar.',
      exportJSON: 'JSON Export',
      aiAnalysis: 'KI Analyse',
      aiModel: 'KI-Modell',
      processingVia: 'Verarbeitung über {{model}}',
      analysisTitle: 'Strategische Analyse',
      generatingAnalysis: 'Erstelle Analyse...',
      analysisError: 'Fehler bei der Analyse.',
      aiSetupTitle: 'KI-Insights einrichten',
      aiSetupBody: 'Schalten Sie strategische Einblicke für Ihre Quartalsplanung mit Gemini AI frei. Bitte aktivieren Sie die KI-Funktionen in den Einstellungen und hinterlegen Sie Ihren API-Key.',
      aiPrivacyTitle: 'Datenschutz & Sicherheit',
      aiPrivacyBody: 'Ihr API-Key wird lokal im Browserspeicher gesichert. Die Verbindung erfolgt direkt zur Google API und wird niemals auf unseren Servern gespeichert.',
      openSettingsInstruction: 'Gehen Sie zu Einstellungen (unten links), um den KI-Zugriff zu konfigurieren.',
      configureSettings: 'Einstellungen konfigurieren',
      monteCarlo: 'Monte Carlo Simulation',
      runSim: 'Starten',
      probability: 'Wahrsch.',
      simTitle: 'Chancen Simulation',
      simSubtitle: 'Wahrscheinlichkeitsgewichtete Kapazitätsanalyse (2000 Iterationen)',
      p90: 'Konservativ (P90)',
      p50: 'Median (P50)',
      p10: 'Aggressiv (P10)',
      simScenarioHigh: 'Hohes Lastszenario',
      simScenarioMed: 'Erwarteter Ausgang',
      simScenarioLow: 'Niedriges Lastszenario',
      netCap: 'Netto Kapazität',
      simDistribution: 'Wahrscheinlichkeitsverteilung',
      riskOfOverload: 'Überlastungsrisiko',
      capacityLimit: 'Kapazitätsgrenze',
      safeZone: 'Sicherer Bereich',
      overloadZone: 'Überlastungsbereich',
    },
    version: {
      title: 'Neue Plan-Revision erstellen',
      name: 'Name der Plan-Revision',
      placeholderName: 'z.B. Angepasster Q4 Plan',
      description: 'Beschreibung',
      placeholderDesc: 'Was hat sich in dieser Revision geändert?',
      cancel: 'Abbrechen',
      create: 'Revision erstellen',
      copyHint: 'Die neue Revision startet als Kopie der neuesten Plan-Revision. Eingefrorene Revisionen sind schreibgeschützt — neue Arbeit verzweigt immer nur von der neuesten.'
    },
    versions: {
      rename: 'Umbenennen',
      delete: 'Löschen',
      deleteTitle: 'Plan-Revision löschen',
      deleteMessage: 'Möchten Sie die Plan-Revision "{{name}}" wirklich löschen?',
      compare: 'Vergleichen',
      compareSelect: 'Basis- und Ziel-Revision wählen',
      diffAdded: 'Hinzugefügt',
      diffRemoved: 'Entfernt',
      diffChanged: 'Geändert',
      diffEmpty: 'Keine Unterschiede gefunden.',
      toastRenamed: 'Plan-Revision erfolgreich umbenannt.',
      toastDeleted: 'Plan-Revision erfolgreich gelöscht.',
      lastVersionGuard: 'Die letzte Plan-Revision kann nicht gelöscht werden.',
      cancel: 'Abbrechen'
    },
    team: {
      title: 'Teamverwaltung',
      subtitle: 'Mitarbeiter, erweitertes Team und Planung',
      addEmployee: 'Mitarbeiter hinzufügen',
      addFutureHire: 'Neue Stelle planen',
      confirmDelete: 'Möchten Sie diesen Mitarbeiter wirklich löschen?',
      editEmployee: 'Mitarbeiter bearbeiten',
      uploadPhoto: 'Foto hochladen',
      name: 'Name',
      placeholderName: 'Vollständiger Name',
      role: 'Rolle',
      placeholderRole: 'z.B. Senior Developer',
      email: 'E-Mail',
      placeholderEmail: 'email@firma.de',
      phone: 'Telefon',
      placeholderPhone: '+49 ...',
      availability: 'Verfügbarkeit',
      location: 'Standort (Feiertagskalender)',
      skills: 'Fähigkeiten',
      typeSkill: 'Skill eingeben und Enter...',
      notes: 'Notizen',
      placeholderNotes: 'Zusätzliche Informationen...',
      saveEmployee: 'Speichern',
      cancel: 'Abbrechen',
      noSkills: 'Keine Skills eingetragen',
      coreTeam: 'Kernteam',
      extendedTeam: 'Erweitertes Team / Andere Bereiche',
      futureHiring: 'Zukünftige Stellen & Planung',
      smartRecruit: 'Smart Recruit',
      smartRecruitSubtitle: 'Interne Vorschläge basierend auf Ressourcenlücken',
      addToTeam: 'Zum Team hinzufügen',
      matchScore: 'Match',
      noSuggestions: 'Keine passenden Kandidaten im Verzeichnis gefunden.',
      open1on1: '1:1 Dashboard öffnen',
      openDevelopment: 'Entwicklung & Standort',
      skillMatrix: 'Skill Matrix',
      directory: 'Verzeichnis',
      delete: 'Löschen',
      deleteTitle: 'Mitarbeiter löschen',
      validation: {
        required: 'Name und Rolle sind Pflichtfelder.'
      }
    },
    admin: {
      title: 'Benutzerverwaltung',
      subtitle: 'Konten und Rollenzuweisungen verwalten',
      addUser: 'Benutzer hinzufügen',
      editUser: 'Benutzer bearbeiten',
      deleteUser: 'Benutzer löschen',
      email: 'E-Mail',
      emailPlaceholder: 'name@firma.de',
      roles: 'Rollen',
      linkedEmployee: 'Verknüpfter Mitarbeiter',
      noEmployeeLink: 'Keine Mitarbeiterverknüpfung',
      cancel: 'Abbrechen',
      saveUser: 'Benutzer speichern',
      deleteTitle: 'Benutzer löschen',
      confirmDelete: 'Möchten Sie diesen Benutzer wirklich löschen? Dies kann nicht rückgängig gemacht werden.',
      delete: 'Löschen',
      noUsers: 'Noch keine Benutzer.',
      retry: 'Erneut versuchen',
      validation: {
        emailRequired: 'E-Mail ist erforderlich.',
        roleRequired: 'Mindestens eine Rolle ist erforderlich.'
      },
      errorInvalidRoles: 'Dieser Benutzer benötigt mindestens eine Rolle.',
      errorDeleteGuard: 'Der letzte Administrator oder das eigene Konto kann nicht gelöscht werden.',
      errorUserNotFound: 'Dieser Benutzer existiert nicht mehr.',
      errorPermissionDenied: 'Sie haben keine Berechtigung, Benutzer zu verwalten.',
      tabs: {
        users: 'Benutzer & Rollen',
        setup: 'Anwendungseinstellungen',
        monitoring: 'System & KPIs'
      },
      setup: {
        title: 'Anwendungseinstellungen',
        subtitle: 'Zur Laufzeit änderbare Anwendungseinstellungen',
        defaultRoleLabel: 'Standardrolle',
        adminEmailsLabel: 'Admin-E-Mails',
        adminEmailsPlaceholder: 'name@firma.de',
        planRevisionRetentionLabel: 'Plan-Revisionen aufbewahren',
        planRevisionRetentionHint: 'Wie viele eingefrorene Plan-Snapshots (Plan-Revisionen) gespeichert bleiben. Wenn eine neue Revision erstellt wird — manuell oder automatisch pro Quartal — werden ältere über dieser Anzahl entfernt.',
        environmentLabel: 'Umgebung',
        environmentEmpty: 'keine konfiguriert',
        overrideBadge: 'DB-Override',
        firstSeenHint: 'Änderungen wirken nur auf Benutzer, die der Server danach zum ersten Mal sieht. Bestehende Konten behalten ihre aktuellen Rollen.',
        save: 'Einstellungen speichern',
        saving: 'Speichern…',
        validationInvalidEmail: 'Jede Zeile muss eine gültige E-Mail-Adresse sein.',
        errorInvalidSettings: 'Der Server hat diese Einstellungen abgelehnt. Bitte prüfen Sie die Werte.'
      },
      monitoring: {
        title: 'System & KPIs',
        subtitle: 'Serverstatus und Datenbankmetriken',
        uptime: 'Laufzeit',
        version: 'Version',
        dbSize: 'Datenbankgröße',
        liveClients: 'Aktive Clients',
        devModeWarning: 'Dev-User-Modus ist aktiv (QFC_DEV_USER) - niemals in der Produktion aktivieren.',
        entitiesTitle: 'Entitäten',
        entities: {
          users: 'Benutzer',
          employees: 'Mitarbeiter',
          customers: 'Kunden',
          projects: 'Projekte',
          planVersions: 'Planversionen',
          assignments: 'Zuordnungen',
          absences: 'Abwesenheiten',
          quarterData: 'Forecast-Quartale',
          strategicGoals: 'Strategische Ziele',
          northStarMetrics: 'North-Star-Metriken',
          oneOnOneSessions: '1:1-Gespräche',
          publicHolidays: 'Feiertage'
        },
        changeLogTitle: 'Change Log',
        changeLogRows: 'Zeilen',
        changeLogSeqRange: 'Sequenzbereich',
        changeLogRetention: 'Retention-Limit',
        lastUpdated: 'Zuletzt aktualisiert',
        autoRefresh: 'Auto-Aktualisierung'
      }
    },
    development: {
      title: 'Standortbestimmung & Entwicklung',
      ikigai: 'Mein Ikigai',
      competencies: 'Kompetenzrad',
      dragDropInstructions: 'Ziehe Elemente in die richtige Zone.',
      addActivity: 'Aktivität hinzufügen',
      zones: {
        love: 'Liebe es',
        good: 'Kann es',
        paid: 'Bezahlt',
        needed: 'Gebraucht'
      },
      gapAnalysis: 'Lückenanalyse',
      self: 'Selbst',
      manager: 'Manager',
      projectSimulator: 'Projekt Simulator',
      simulatorDesc: 'Prüfe Team-Bereitschaft für ein potenzielles Projekt.',
      requiredSkill: 'Benötigter Skill',
      requiredLevel: 'Benötigtes Level (1-5)',
      checkReadiness: 'Bereitschaft prüfen',
      ready: 'Bereit',
      gap: 'Lücke',
      qualified: 'Qualifiziert',
      skillGap: 'Skill Lücke'
    },
    customers: {
      title: 'Kundenübersicht',
      subtitle: 'Kundenportfolio und Status',
      addCustomer: 'Kunde hinzufügen',
      newCustomer: 'Neuer Kunde',
      editCustomer: 'Kunde bearbeiten',
      deleteCustomer: 'Kunde löschen',
      customerName: 'Kundenname',
      logo: 'Logo URL',
      industry: 'Branche',
      contactName: 'Ansprechpartner',
      email: 'E-Mail',
      notes: 'Notizen',
      saveCustomer: 'Kunde speichern',
      cancel: 'Abbrechen',
      confirmDelete: 'Möchten Sie diesen Kunden wirklich löschen?',
      hrCoverage: 'HR Abdeckung',
      days: 'Tage',
      budgetCoverage: 'Budget Abdeckung',
      projects: 'Aktive Projekte',
      planned: 'Geplant',
      accountsCount: '{{count}} Beauftragungen',
      gap: 'Lücke',
      onTrack: 'Im Plan',
      noProjects: 'Keine aktiven Projekte für diesen Kunden.',
      delete: 'Löschen',
      deleteTitle: 'Kunden löschen',
      validation: {
        required: 'Kundenname und E-Mail sind Pflichtfelder.'
      }
    },
    financials: {
        title: 'Finanzübersicht',
        subtitle: 'Umsatzprognosen und Projektrentabilität',
        revenueForecast: 'Umsatzprognose (4 Quartale)',
        profitability: 'Projektrentabilität & Margen',
        project: 'Projekt',
        budget: 'Budget',
        plannedCost: 'Geplante Kosten',
        margin: 'Marge',
        status: 'Status',
        all: 'Alle',
        highRisk: 'Hohes Risiko',
        healthy: 'Gesund',
        low: 'Gering',
        lowMargin: 'Geringe Marge',
        risk: 'Risiko',
        drawable: 'Abrufbar',
        overrun: 'Überzogen',
        account: 'Beauftragung',
        totalRevenue: 'Gesamtumsatz',
        avgMargin: 'Ø Marge',
        noProjectsFound: 'Keine Projekte entsprechen Ihren Kriterien.',
        noProjects: 'Noch keine Projekte vorhanden. Legen Sie ein Projekt an, um hier Finanzdaten zu sehen.',
        noBudgetsSet: 'Für keines Ihrer Projekte ist ein Budget hinterlegt. Hinterlegen Sie ein Budget, um die Rentabilität zu sehen.',
        noProjectDates: 'Hinterlegen Sie Start- und Enddaten für Ihre Projekte, damit sie in der Umsatzprognose berücksichtigt werden.',
        revenueNoData: 'Noch keine Umsatzprognose-Daten.',
        revenueNoDataHint: 'Der Umsatz wird aus den Projektbudgets über deren Start–Ende-Zeitraum verteilt. Hinterlegen Sie bei mindestens einem Projekt Budget sowie Start- und Enddatum, dann erscheint das Quartalsdiagramm hier.'
    },
    strategy: {
        title: 'Strategie & Vision',
        subtitle: 'Projekte an Unternehmenszielen und North-Star-Metriken ausrichten',
        strategyMap: 'Strategiekarte (Balanced Scorecard)',
        northStar: 'North-Star-Ausrichtung',
        aiCoPilot: 'KI Strategie Co-Pilot',
        perspectives: {
            financial: 'Finanzen',
            customer: 'Kunden',
            internal: 'Interne Prozesse',
            learning: 'Lernen & Entwicklung'
        },
        addGoal: 'Ziel hinzufügen',
        newGoal: 'Neues Ziel',
        goalTitle: 'Zielbezeichnung',
        goalPerspective: 'Perspektive',
        saveGoal: 'Ziel speichern',
        cancel: 'Abbrechen',
        sunburstTitle: 'Arbeitsvolumen vs Strategische Metriken',
        sunburstSubtitle: 'Innerer Ring: Metrik-Kategorien | Äußerer Ring: Beitragende Projekte',
        totalVolume: 'Gesamtvolumen',
        maintenance: 'Wartung & Sonstiges',
        coPilotTitle: 'Strategieberater',
        coPilotSubtitle: 'Ich helfe Ihnen, Ihr Strategiedokument zu entwerfen.',
        startInterview: 'Interview starten',
        generateDoc: 'Dokument generieren',
        interviewMode: 'INTERVIEW MODUS',
        docMode: 'DOKUMENTEN MODUS',
        aiDisabled: 'Bitte aktivieren Sie KI in den Einstellungen und geben Sie einen API-Key ein.',
        aiError: 'Fehler bei der Verbindung zum KI-Strategieberater.',
        greeting: 'Ich helfe Ihnen, Ihr Strategiedokument zu entwerfen. Strategie-Interview starten?',
        configureAi: 'KI konfigurieren'
    },
    sales: {
        title: 'Sales Pipeline',
        subtitle: 'Leads und Verkaufschancen verwalten',
        addLead: 'Lead hinzufügen',
        marketTrends: 'Markttrends',
        pipelineValue: 'Pipeline Wert',
        weightedValue: 'Gewichteter Wert',
        activeLeads: 'Aktive Leads',
        stages: {
            lead: 'Neuer Lead',
            qualified: 'Qualifiziert',
            proposal: 'Angebot',
            negotiation: 'Verhandlung'
        },
        marketScoutTitle: 'Markt Trend Scout',
        marketScoutSubtitle: 'KI nutzen um Potenziale zu finden',
        trendsPrompt: 'Welche Branche oder Kunde soll analysiert werden?',
        analyze: 'Trends analysieren',
        analyzing: 'Suche Marktdaten...',
        aiNotConfigured: 'KI nicht konfiguriert. Prüfen Sie die Einstellungen.',
        aiErrorTrends: 'Fehler bei der Trendanalyse. Prüfen Sie den API-Key.',
        placeholderProjectName: 'z.B. Website Relaunch',
        placeholderClient: 'Kundenname'
    },
    chat: {
      assistantReady: 'ASSISTENT_BEREIT',
      resourceAssistant: 'Ressourcen_Assistent',
      howCanIHelp: 'Wie kann ich bei der Planung helfen?',
      askAbout: 'Fragen Sie nach Kapazität, Konflikten oder Projekten.',
      accessDenied: 'ZUGRIFF VERWEIGERT: KI-Modul nicht konfiguriert. Bitte API-Key in Einstellungen setzen.',
      errorComputation: 'FEHLER: Berechnung fehlgeschlagen. Bitte wiederholen.',
      sendError: 'Nachricht konnte nicht gesendet werden. Bitte Verbindung prüfen und erneut versuchen.',
      inputPlaceholder: 'Fragen zu Ressourcen, Projekten oder Konflikten...',
      aiConfigMissing: 'KI-Konfiguration fehlt',
      enableInSettings: 'In Einstellungen aktivieren',
      you: 'Du',
      assistant: 'Assistent',
      suggestion1: 'Zeige überlastete Mitarbeiter',
      suggestion2: 'Projekte, die in Q4 enden',
      suggestion3: 'Wer hat nächsten Monat Kapazität?',
      suggestion4: 'Gibt es Ressourcenkonflikte?',
      settings: 'Einstellungen',
      minimize: 'Minimieren',
      close: 'Schließen',
      restore: 'Chat wiederherstellen',
      triggerLabel: 'Ressourcen-Assistent öffnen',
      send: 'Nachricht senden',
      confirmCloseTitle: 'Assistent schließen',
      confirmCloseMessage: 'Beim Schließen des Assistenten wird die aktuelle Konversation gelöscht. Möchten Sie fortfahren?',
      confirmCloseConfirm: 'Schließen',
      confirmCloseCancel: 'Abbrechen'
    },
    status: {
      active: 'Aktiv',
      opportunity: 'Chance',
      completed: 'Abgeschlossen',
      on_hold: 'Pausiert'
    },
    toast: {
      employeeSaved: 'Mitarbeiter erfolgreich gespeichert.',
      employeeDeleted: 'Mitarbeiter erfolgreich gelöscht.',
      projectSaved: 'Projekt erfolgreich gespeichert.',
      projectDeleted: 'Projekt erfolgreich gelöscht.',
      customerSaved: 'Kunde erfolgreich gespeichert.',
      customerDeleted: 'Kunde erfolgreich gelöscht.',
      versionCreated: 'Neue Plan-Revision erfolgreich erstellt.',
      planCreated: 'Resourceplan erstellt',
      csvExported: 'CSV erfolgreich exportiert.',
      userSaved: 'Benutzer erfolgreich gespeichert.',
      userDeleted: 'Benutzer erfolgreich gelöscht.',
      settingsSaved: 'Einstellungen erfolgreich gespeichert.'
    }
  }
};

export type Language = keyof typeof translations;