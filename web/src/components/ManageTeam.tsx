...
export const ManageTeam: React.FC<ManageTeamProps> = ({
  ...
  const defaultTypeRef = useRef<'internal' | 'future'>('internal');

  const makeDefaults = useCallback(
    (): Partial<Employee> => {
      const isFuture = defaultTypeRef.current === 'future';
      return {
        name: isFuture ? 'TBD' : '',
        role: '',
        avatar: isFuture ? '' : `https://ui-avatars.com/api/?name=New+User&background=random`,
        skills: [],
        availability: 100,
        email: '',
        phone: '',
    ...
    validate: useCallback(
      (data) => {
        if (!data.name?.trim() || !data.role?.trim()) {
          return t('team.validation.required');
        }
        return null;
      },
      [t]
    ),
    onAfterSave: useCallback(() => success(t('toast.employeeSaved')), [success, t]),
    onAfterDelete: useCallback(() => success(t('toast.employeeDeleted')), [success, t])
  });
  ...
  const handleAdd = (type: 'internal' | 'future') => {
    defaultTypeRef.current = type;
    openAdd();
  };
  ...
  const handleRecruit = (dirEmp: Employee) => {
    const newEmp = { ...dirEmp, type: 'external' as const };
  ...
  const buildEmployee = (data: Partial<Employee>, editingId: string | null): Employee => {
    ...
    return {
      ...
      name,
      role: (data.role ?? '').trim(),
      avatar: isFuture ? '' : (data.avatar?.trim() || `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'New User')}&background=random`),
      skills: data.skills ?? [],
      availability: data.availability ?? 100,
      email: data.email?.trim(),
      phone: data.phone?.trim(),
      notes: data.notes?.trim(),
      location: data.location ?? 'DE',
  ...
  return (
    <div className="h-full overflow-auto bg-gray-50/50 p-6 custom-scrollbar">
      ...
      {/* Edit Employee Modal */}
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? t('team.editEmployee') : (formData.type === 'future' ? t('team.addFutureHire') : t('team.addEmployee'))} size="lg">
        <form onSubmit={handleSubmit(buildEmployee)} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="flex justify-end pt-4 gap-3">
              <Button type="button" variant="ghost" onClick={closeModal}>{t('team.cancel')}</Button>
